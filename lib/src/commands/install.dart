...
class InstallCommand {
  ...
  install(
          @Rest(required: true, valueHelp: 'endpoint', allowed: packageList,
...
''')
      ...
      Pubspec.load().then((pubspec) {
        ...
        new Future(() {
          return Future.wait(packages.map(_SplitPackage.parse).map(
              (splitPackage) =>
                  splitPackage.getPackageDep(source, caret: caret)));
        }).then((deps) {
          ...
          deps.forEach((PackageDep dep) {
            ...
            buffer
              ...
              ..write(theme.version(dep.source == 'hosted'
                  ? "'${dep.constraint}'"
                  : dep.description));
            if (movedDependencies.containsKey(dep.name))
              buffer.write(theme.info(' (moved from "$otherDepGroupKey")'));
            lines.add(buffer.toString());
          });
          print(
              block('Installed these ${dev ? 'dev_' : ''}dependencies', lines));
        });
      });
}
...
class _SplitPackage {
  ...
  static var packagePattern =
      new RegExp(r'^(([a-zA-Z0-9_]+)=)?([^#]+)(#([^#]+))?');

  static _SplitPackage parse(String package) {
    var match = packagePattern.firstMatch(package);
    if (match == null)
      throw new FormatException('Invalid package argument', package);
    return new _SplitPackage(
        match.input, match.group(2), match.group(3), match.group(5));
  }

  _SplitPackage(this.input, this.explicitName, this.body, this.hash);

  Future<PackageDep> getPackageDep(String source, {bool caret}) =>
      new Future(() {
        if (source == 'hosted') {
          // <name>#<version constraint>.
          if (!nullOrEmpty(explicitName))
            throw new FormatException(
                'Cannot specify explicit name for hosted dependency', input);
          var name = body;
          return new Future(() {
        ...
        if (source == 'path') {
          ...
          if (!nullOrEmpty(hash))
            throw new FormatException(
                'Cannot specify hash fragment for path dependency', input);
          var path = body;
...
//            } catch (e) { }
        ...
        if (source == 'git') {
          ...
          if (name == null) {
            name =
                p.basenameWithoutExtension(Uri.parse(gitUri).pathSegments.last);
          }
          var description = ref == null ? gitUri : {'url': gitUri, 'ref': ref};
