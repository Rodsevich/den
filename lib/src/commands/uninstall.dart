...
class UninstallCommand {
  ...
  uninstall(
          @Rest(required: true, valueHelp: 'package name', allowed: getImmediateDependencyNames, help: 'Name of dependency to remove')
              List<String> names) =>
      Pubspec.load().then((pubspec) {
        ...
        if (removedDeps.isNotEmpty) {
          ...
          removedDeps.forEach((name, old) {
            lines.add(
                '${theme.dependency(name)}${theme.info(': ')}${theme.version(JSON.encode(old))}');
          });
